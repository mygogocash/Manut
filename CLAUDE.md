...
## 5e. Knowledge Graph activation pulses (in-flight reference)

> **Status as of v1.12.0:** the activation-pulses redesign of the
> Knowledge Graph is implemented on
> `feat/superflow-pm-crm-reminders-ui` but did NOT merge into `main`
> for v1.12.0. This section is a quick reference for the doc-read
> event bus contract so the next session can pick it up cold without
> re-discovering the design. If you find this section but the code
> hasn't shipped, treat it as a design document for the in-flight
> branch.
...
When this lands on `main`, port this section into the trap list in
§5 and delete the "in-flight" framing.
