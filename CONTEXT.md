...
- **Investor CRM (`/investors`)** is now a Sales-CRM-style tabbed workspace — tabs: Pipeline · Investors · Leads · Accounts · Contacts · Activities · Tasks (no Dashboard tab; its KPIs moved to per-column pipeline totals). Notable pieces:
  ...
  - **Configurable investor types** (`InvestorTypeOption` + `/api/investor/types`) — Family Office / PE / VC / SWF / Corporate / State Capital / Growth / Individual, seeded from the TBH Pipeline Master sheet; the type column is an open key fed through a `useInvestorTypes` hook + Manage types dialog.
