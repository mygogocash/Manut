...
describe("visa dual-path routes", () => {
  ...
  it("strips storage URLs and employee email from visa Hyperdrive list projections", async () => {
    const documentUrl =
      "https://files.manut.example/storage/v1/object/private/visa/secret.pdf";
    const store = memoryStore({
      records: [
        {
          id: "visa-own",
    ...
    const documents = (row.documents as Array<Record<string, unknown>>) ?? [];
    ...
    expect(row).not.toHaveProperty("documentUrl");
