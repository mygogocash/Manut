...
export function SurveyRespondScreen() {
  ...
  const [submitted, setSubmitted] = useState<SubmittedSurveyResponse | null>(
    null,
  );
  ...
  const submitMutation = useMutation({
    mutationFn: () => {
      if (!detailQuery.data || !id) {
        throw new Error("Survey is not loaded yet.");
      }
      const built = buildAnswersPayload(detailQuery.data, drafts);
      const parsed = submitSurveyResponseInputSchema.safeParse(built);
      if (!parsed.success) {
        const issue = parsed.error.issues[0];
        throw new Error(issue?.message ?? "Check your answers and try again.");
      }
      return submitSurveyResponse(api, id, parsed.data);
    },
    onSuccess: (receipt) => {
