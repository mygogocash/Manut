...
function errorMessage(error: unknown): string {
  return error instanceof ApiError
    ? error.message
    : "We could not load the leave approval chain.";
}

export function LeaveApprovalScreen() {
  const api = useApiClient();
  const stepsQuery = useQuery({
    queryKey: LEAVE_APPROVAL_STEPS_QUERY_KEY,
    queryFn: ({ signal }) => listLeaveApprovalSteps(api, signal),
  });
      ...
      <View style={{ width: "100%", maxWidth: 720, gap: spacing.lg }}>
        <View style={{ gap: spacing.xs }}>
          ...
          <Text selectable style={{ color: colors.textMuted }}>
            Read-only view of org-wide leave approval steps. Reorder and edit
            remain on admin tools for now.
          </Text>
        </View>
        ...
        {stepsQuery.isError ? (
          <Card title="Approval chain unavailable">
            ...
            <Button
              ...
              accessibilityLabel="Retry leave approval steps"
        ...
        {stepsQuery.data ? (
          stepsQuery.data.length === 0 ? (
            <Card title="No approval steps">
              <Text selectable style={{ color: colors.textMuted }}>
                No leave approval steps are configured yet.
              </Text>
            </Card>
          ) : (
            <View
              accessibilityLabel="Leave approval steps"
            ...
            >
              {stepsQuery.data.map((step) => (
                <Card
                  ...
                  description={leaveApproverTypeLabel(step.approverType)}
                >
                  <View style={{ gap: spacing.xs }}>
                    <Text selectable style={{ color: colors.textMuted }}>
