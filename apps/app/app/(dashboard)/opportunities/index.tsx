...
type Item = { id: string; name?: string; stage?: string; value?: number; currency?: string };
...
export default function OpportunitiesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/opportunities?page=1&limit=50`, {
          credentials: "include",
        });
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>
              {item.stage ?? "—"}
              {item.value != null ? ` · ${item.currency ?? ""} ${item.value}` : ""}
            </Text>
          </View>
        )}
      />
