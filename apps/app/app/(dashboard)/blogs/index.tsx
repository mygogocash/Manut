...
type Blog = { id: string; title: string; content: string; author?: { name?: string }; active: boolean };
...
export default function BlogsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/blogs?page=1&limit=20`, { credentials: "include" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = (await res.json()) as { data: Blog[] };
  ...
  return (
    <View style={styles.root}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.sub}>No blogs yet.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.title}</Text>
            <Text style={styles.meta}>{item.author?.name ?? "Intranet"}</Text>
...
const styles = StyleSheet.create({
  ...
  body: { color: "#5C4A3A", lineHeight: 20 },
});
