...
export default function HelpdeskPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/helpdesk?scope=mine&limit=50`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.sub}>No tickets yet.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>
              {[item.status, item.priority, item.category].filter(Boolean).join(" · ") || "—"}
            </Text>
          </View>
        )}
      />
