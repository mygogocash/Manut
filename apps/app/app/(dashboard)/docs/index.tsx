...
export default function DocsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/docs?page=1&limit=50`, { credentials: "include" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = (await res.json()) as { data: WikiPage[] };
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>
              {[item.folder, item.slug, item.isPublished === false ? "Draft" : null]
                .filter(Boolean)
                .join(" · ") || "—"}
            </Text>
          </View>
