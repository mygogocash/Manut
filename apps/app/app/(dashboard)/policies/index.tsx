...
export default function PoliciesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/policies`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.sub}>No policies yet.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.title}</Text>
            <Text style={styles.meta}>
              {item.category}
              {item.entity?.code ? ` · ${item.entity.code}` : " · Global"} · {item.fileName}
            </Text>
          </View>
        )}
      />
