...
type Item = { id: string; name?: string; type?: string; status?: string };
...
export default function InvestorsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/investors?page=1&limit=50`, { credentials: "include" });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.name ?? item.id}</Text>
            <Text style={styles.meta}>{[item.type, item.status].filter(Boolean).join(" · ") || "—"}</Text>
          </View>
        )}
      />
    </View>
