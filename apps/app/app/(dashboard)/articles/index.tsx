...
type Article = { id: string; title: string; date: string; link: string; author?: { name?: string } };
...
export default function ArticlesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/articles?page=1&limit=20`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>PR Articles</Text>
      {error ? <Text style={styles.error}>{error}</Text> : null}
      <FlatList
        data={items}
        keyExtractor={(item) => item.id}
        ListEmptyComponent={<Text style={styles.sub}>No articles yet.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline} onPress={() => Linking.openURL(item.link)}>
              {item.title}
            </Text>
...
const styles = StyleSheet.create({
  ...
  headline: { fontSize: 18, fontWeight: "600", color: "#8B6914" },
