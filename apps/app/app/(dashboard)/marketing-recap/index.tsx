...
export default function MarketingRecapPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/marketing-recap/targets`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>Marketing Recap</Text>
      {error ? <Text style={styles.error}>{error}</Text> : null}
      <FlatList
        data={rows}
        keyExtractor={(item) => item.id}
        ListEmptyComponent={<Text style={styles.sub}>No targets yet.</Text>}
        renderItem={({ item }) => (
