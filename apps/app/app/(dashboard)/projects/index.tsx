...
type Item = { id: string; name?: string; team?: string; status?: string; slug?: string };
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/projects?limit=50`, { credentials: "include" });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.name ?? item.slug ?? item.id}</Text>
