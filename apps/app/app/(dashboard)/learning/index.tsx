...
type Item = { id: string; title?: string; name?: string; category?: string; department?: string };
...
export default function LearningPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/learning/modules?page=1&limit=50`, { credentials: "include" });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.title ?? item.name ?? item.id}</Text>
