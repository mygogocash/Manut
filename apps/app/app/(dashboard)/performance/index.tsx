...
type Item = { id: string; title?: string; name?: string; status?: string; type?: string };
...
export default function PerformancePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/performance/cycles?page=1&limit=50`, { credentials: "include" });
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>
              {item.name ?? item.title ?? item.name ?? item.id}
            </Text>
