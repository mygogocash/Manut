...
type Item = { id: string; name?: string; label?: string; code?: string };
...
export default function CrmActivitiesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/crm/activities`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>CRM Activities</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.name ?? item.label ?? item.code ?? item.id}</Text>
          </View>
        )}
      />
    </View>
