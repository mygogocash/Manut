...
type Item = { id: string; visaType?: string; country?: string; holderName?: string; status?: string };
...
export default function VisaPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/visa?page=1&limit=50`, { credentials: "include" });
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>
              {item.holderName ?? item.visaType ?? item.id}
            </Text>
            <Text style={styles.meta}>
              {[item.country, item.status].filter(Boolean).join(" · ") || "—"}
            </Text>
          </View>
        )}
      />
