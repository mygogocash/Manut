...
type Item = { id: string; title?: string; name?: string; holderName?: string; currency?: string };

export default function VisaKbPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/visa-kb?page=1&limit=50`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>Visa Knowledge Base</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.title ?? item.name ?? item.holderName ?? item.id}</Text>
