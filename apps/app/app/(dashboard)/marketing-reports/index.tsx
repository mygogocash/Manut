...
export default function MarketingReportsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/marketing-reports/dashboard?days=30`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>Marketing Reports</Text>
      {error ? <Text style={styles.error}>{error}</Text> : null}
      <FlatList
        data={rows}
        keyExtractor={(item) => item.id}
        ListEmptyComponent={<Text style={styles.sub}>No metrics yet.</Text>}
        renderItem={({ item }) => (
