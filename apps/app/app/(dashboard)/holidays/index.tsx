  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${getAppUrl()}/api/holidays?page=1&limit=100&year=${year}`, {
          credentials: "include",
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = (await res.json()) as { data: Holiday[] };
  ...
  return (
    <View style={styles.root}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.sub}>No holidays for this year.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.name}</Text>
            <Text style={styles.meta}>
              {item.date}
              {item.entity?.code ? ` · ${item.entity.code}` : ""}
            </Text>
          </View>
        )}
      />
