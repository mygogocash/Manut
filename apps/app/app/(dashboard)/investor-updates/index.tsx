...
type Item = { id: string; title?: string; period?: string; status?: string };
...
export default function InvestorUpdatesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/investor-updates?page=1&limit=50`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>Investor Updates</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>{[item.period, item.status].filter(Boolean).join(" · ") || "—"}</Text>
          </View>
        )}
      />
    </View>
