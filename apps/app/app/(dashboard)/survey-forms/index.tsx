...
type Item = { id: string; title?: string; status?: string; alreadyResponded?: boolean };
...
export default function SurveyFormsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/survey-forms?scope=available&page=1&limit=50`, { credentials: "include" });
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.title}>Survey Forms</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.headline}>{item.title ?? item.id}</Text>
            <Text style={styles.meta}>
              {[item.status, item.alreadyResponded ? "Responded" : null].filter(Boolean).join(" · ") || "—"}
            </Text>
          </View>
        )}
      />
