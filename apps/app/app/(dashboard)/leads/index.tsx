...
type Item = { id: string; name?: string; company?: string; status?: string };
...
export default function LeadsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/leads?page=1&limit=50`, { credentials: "include" });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>{item.company ?? item.status ?? "—"}</Text>
          </View>
        )}
      />
    </View>
