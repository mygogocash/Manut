...
type Item = { id: string; name?: string; domain?: string; country?: string };
...
export default function AccountsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${getAppUrl()}/api/accounts?page=1&limit=50`, { credentials: "include" });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>{item.domain ?? item.country ?? "—"}</Text>
          </View>
        )}
      />
    </View>
