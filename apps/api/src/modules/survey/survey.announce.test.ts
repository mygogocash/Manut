...
const TITLE = "Go the Extra Mile Survey";
const RESPOND_LINK = `/survey/${FORM_ID}/respond`;
const PERMS = [
...
describe("announceNow — idempotent repair-or-create", () => {
  it("creates fresh cards when none exist, linking each to the respond page", async () => {
    ...
    const res = await surveyService.announceNow(
      FORM_ID,
      CREATOR,
      PERMS,
      ANNOUNCE,
  ...
  it("is a no-op when a card is already linked (no duplicate, no create)", async () => {
    ...
    await surveyService.announceNow(FORM_ID, CREATOR, PERMS, ANNOUNCE);
