...
describe("AccountService", () => {
  ...
  describe("getById", () => {
    it("hides accounts owned by other reps", async () => {
      ...
      await expect(
        service.getById("acc-1", USER_ID, ["sales-revenue:read"]),
  ...
  describe("create — account dedupe", () => {
    ...
    it("returns 409 candidate on case-insensitive name match without confirmCreate", async () => {
      ...
      await expect(
        service.create(USER_ID, ["sales-revenue:create"], { name: "ACME" }),
      ).rejects.toThrow(ConflictException);
      expect(create).not.toHaveBeenCalled();
    });
  ...
  describe("update", () => {
    it("rejects when domain collides with a different account", async () => {
      ...
      await expect(
        service.update("acc-1", USER_ID, ["sales-revenue:update"], {
          domain: "acme.com",
        }),
      ).rejects.toThrow(ConflictException);
    });
    ...
    it("allows the same domain on the same account", async () => {
      ...
      await service.update("acc-1", USER_ID, ["sales-revenue:update"], {
        domain: "acme.com",
      });
  ...
  describe("delete", () => {
    it("blocks delete when caller does not own the row", async () => {
      ...
      await expect(
        service.delete("acc-1", USER_ID, ["sales-revenue:delete"]),
      ).rejects.toThrow(NotFoundException);
