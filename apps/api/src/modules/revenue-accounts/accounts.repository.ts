...
} satisfies Prisma.RevenueAccountInclude;
...
export class AccountRepository {
  async findMany(filters: ListAccountsFilters, page: number, limit: number) {
    ...
    const [data, total] = await Promise.all([
      prisma.revenueAccount.findMany({
        where,
      ...
      prisma.revenueAccount.count({ where }),
    ]);
  ...
  async findById(id: string) {
    return prisma.revenueAccount.findUnique({
      where: { id },
      include: accountInclude,
    });
  }
  ...
  async findByNameInsensitive(name: string) {
    return prisma.revenueAccount.findFirst({
      where: { name: { equals: name, mode: "insensitive" } },
      select: { id: true, name: true, domain: true },
    });
  }

  async create(data: Prisma.RevenueAccountCreateInput) {
    return prisma.revenueAccount.create({ data, include: accountInclude });
  }

  async update(id: string, data: Prisma.RevenueAccountUpdateInput) {
    return prisma.revenueAccount.update({
      where: { id },
      data,
      include: accountInclude,
    });
  ...
  async delete(id: string) {
    return prisma.revenueAccount.delete({ where: { id } });
  }

  async reorder(orderedIds: string[]) {
    await prisma.$transaction(
      orderedIds.map((id, index) =>
        prisma.revenueAccount.update({
          where: { id },
  ...
  async findIdsByOwner(ids: string[], ownerId: string) {
    return prisma.revenueAccount.findMany({
