...
export const attendanceAnalyticsService = {
  async getSummary(
    ...
    const remotePercentage =
      presentCount > 0
        ? Math.round((remoteCount / presentCount) * 1000) / 10
        : 0;
    const hybridPercentage =
      presentCount > 0
        ? Math.round((hybridCount / presentCount) * 1000) / 10
        : 0;
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const mWorking = await attendanceCalendarService.countWorkingDaysInRange(
        mFrom,
        mTo,
        entityId,
      );
    ...
    const deptMap = new Map<
      string,
      { present: number; late: number; possible: number }
    >();

    for (const emp of employees) {
      const dept = emp.department?.trim() || "Unassigned";
      if (!deptMap.has(dept)) {
        deptMap.set(dept, { present: 0, late: 0, possible: 0 });
      }
    ...
    const departmentBreakdown = [...deptMap.entries()].map(
      ([department, d]) => {
        // "absent" is virtual (never persisted): derive it from the working-
        // day capacity minus the present-type days, not a stored status.
        const absent = Math.max(0, d.possible - d.present);
        return {
          department,
          attendancePercentage:
            d.possible > 0
              ? Math.round((d.present / d.possible) * 1000) / 10
              : 0,
          latePercentage:
            d.present > 0 ? Math.round((d.late / d.present) * 1000) / 10 : 0,
          absentPercentage:
