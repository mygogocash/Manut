...
import { applySatangAdjustment, roundMoney } from "@/modules/accounting/rounding";
...
/** Line discount then header discount (average_before_vat). Header remainder
 *  lands on the largest net-before-header line. VAT is on the discounted base. */
export function computeArDocument(
  ...
  const netSum = roundMoney(
    prepared.reduce((s, l) => s + l.netBeforeHeader, 0),
  );
  ...
  // Remainder of header onto the largest tax base if pennies remain from
  // the last-line assignment — already handled by last-line residual.
  ...
  if (grandTotal < 0) {
    throw new BadRequestException("Document total cannot be negative");
  }
