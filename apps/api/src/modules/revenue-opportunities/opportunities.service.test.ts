...
import { opportunityRepository } from "@/modules/revenue-opportunities/opportunities.repository";
import { OpportunityService } from "@/modules/revenue-opportunities/opportunities.service";
...
vi.mock("@/modules/revenue-opportunities/opportunities.repository", () => ({
...
describe("OpportunityService", () => {
  ...
  describe("pipeline", () => {
    it("delegates to repo with proper scope", async () => {
      ...
      await service.pipeline(USER_ID, [
        "sales-revenue:read",
  ...
  describe("update — stage / probability interplay", () => {
    it("snaps probability on stage change when probabilityCustom = false", async () => {
      ...
      await service.update("opp-1", USER_ID, ["sales-revenue:update"], {
        stage: "proposal",
      });
  ...
  describe("closeLost", () => {
    it("flips stage and zeroes probability even when probabilityCustom was true", async () => {
      ...
      await service.closeLost("opp-1", USER_ID, ["sales-revenue:update"], {
    ...
    it("rejects when already closed_lost", async () => {
      ...
      await expect(
        service.closeLost("opp-1", USER_ID, ["sales-revenue:update"], {}),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("reopen", () => {
    it("flips closed_lost back to qualified and snaps probability when not custom", async () => {
      ...
      await service.reopen("opp-1", USER_ID, ["sales-revenue:update"], {
        stage: "qualified",
      });
    ...
    it("rejects when opportunity is still in the live pipeline", async () => {
      ...
      await expect(
        service.reopen("opp-1", USER_ID, ["sales-revenue:update"], {
          stage: "qualified",
        }),
      ).rejects.toThrow(BadRequestException);
      expect(update).not.toHaveBeenCalled();
  ...
  describe("getById / delete ownership", () => {
    it("hides opportunities owned by other reps", async () => {
      ...
      await expect(
        service.getById("opp-1", USER_ID, ["sales-revenue:read"]),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("blocks delete when caller does not own the row", async () => {
      ...
      await expect(
        service.delete("opp-1", USER_ID, ["sales-revenue:delete"]),
      ).rejects.toThrow(NotFoundException);
  ...
  describe("reorderWithinStage", () => {
    it("renumbers the whole stage with submitted cards on top", async () => {
      ...
      const res = await service.reorderWithinStage(
        USER_ID,
    ...
    it("owner-scopes the lookup for a rep without crm:team-read", async () => {
      ...
      await service.reorderWithinStage(USER_ID, ["sales-revenue:update"], {
        stageKey: "qualified",
        orderedIds: ["a"],
      });
    ...
    it("does not owner-scope for crm:team-read callers", async () => {
      ...
      await service.reorderWithinStage(
        USER_ID,
        ["sales-revenue:update", "sales-revenue:team-read"],
    ...
    it("hides a foreign or missing id as NotFound (no enumeration oracle)", async () => {
      ...
      await expect(
        service.reorderWithinStage(USER_ID, ["sales-revenue:update"], {
          stageKey: "qualified",
          orderedIds: ["foreign"],
        }),
      ).rejects.toThrow(NotFoundException);
  ...
  describe("forecast", () => {
    ...
    it("scopes to caller without crm:team-read", async () => {
      ...
      await service.forecast(USER_ID, ["sales-revenue:read"], "USD");
