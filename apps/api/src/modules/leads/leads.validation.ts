...
// `source` was a fixed Zod enum and is now backed by
...
const REP_SETTABLE_STATUSES = ["new", "contacted", "qualified"] as const;

// The stale-lead threshold is hard-coded to 14 days and can be promoted
// to a workspace-configurable setting in a future release.
...
// Lead-conversion body. Account / contact pieces are optional — when both
...
export const convertLeadSchema = z
  .object({
    ...
    // Gated by `crm:reassign` at the service layer.
    ownerId: z.string().optional(),
    // Fallback override forwarded to account dedupe.
