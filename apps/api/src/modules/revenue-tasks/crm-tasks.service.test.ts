...
import { crmTaskRepository } from "@/modules/revenue-tasks/crm-tasks.repository";
import { CrmTaskService } from "@/modules/revenue-tasks/crm-tasks.service";
import { mockArgument } from "@/test-utils/assertions";

vi.mock("@/modules/revenue-tasks/crm-tasks.repository", () => ({
  crmTaskRepository: {
    findMany: vi.fn(),
    findById: vi.fn(),
    create: vi.fn(),
...
describe("CrmTaskService", () => {
  ...
  describe("update", () => {
    it("stamps completedAt when status flips to done", async () => {
      ...
      await service.update("task-1", USER_ID, ["sales-revenue:update"], {
        status: "done",
      });
    ...
    it("rejects edits on a cancelled task", async () => {
      ...
      await expect(
        service.update("task-1", USER_ID, ["sales-revenue:update"], {
          subject: "x",
        }),
  ...
  describe("complete", () => {
    it("is idempotent on already-done rows", async () => {
      ...
      const result = await service.complete("task-1", USER_ID, [
    ...
    it("rejects completing a cancelled task", async () => {
      ...
      await expect(
        service.complete("task-1", USER_ID, ["sales-revenue:update"]),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("flips status and stamps completedAt for an open task", async () => {
      ...
      await service.complete("task-1", USER_ID, ["sales-revenue:update"]);
  ...
  describe("delete", () => {
    it("blocks delete when caller does not own the row", async () => {
      ...
      await expect(
        service.delete("task-1", USER_ID, ["sales-revenue:delete"]),
