...
describe("ContactService", () => {
  ...
  describe("create", () => {
    ...
    it("auto-promotes the first contact to primary even without isPrimary flag", async () => {
      ...
      const result = await service.create(USER_ID, ["crm:create"], {
        accountId: "acc-1",
        firstName: "Jane",
        lastName: "Doe",
      });
  ...
  describe("getById", () => {
    it("hides contacts on accounts owned by other reps", async () => {
      ...
      await expect(
        service.getById("contact-1", USER_ID, ["crm:read"]),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("update", () => {
    it("clears primary on siblings when promoting", async () => {
      ...
      await service.update("contact-1", USER_ID, ["crm:update"], {
        isPrimary: true,
      });
  ...
  describe("delete", () => {
    it("rejects when caller cannot see the parent account", async () => {
      ...
      await expect(
        service.delete("contact-1", USER_ID, ["crm:delete"]),
      ).rejects.toThrow(NotFoundException);
