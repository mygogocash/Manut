...
vi.mock("@/infrastructure/database/prisma", () => ({
  prisma: {
    revenueLeadSource: { findUnique: vi.fn() },
    revenueLead: { findMany: vi.fn() },
  },
}));
...
const findLeadSource = prisma.revenueLeadSource.findUnique as Mock;
const findLeadsRaw = prisma.revenueLead.findMany as Mock;
...
describe("LeadService", () => {
  ...
  describe("list", () => {
    it("scopes to caller without crm:team-read", async () => {
      ...
      const result = await service.list(USER_ID, ["sales-revenue:read"], {
        page: 1,
        limit: 20,
      });
  ...
  describe("listStale", () => {
    it("scopes to caller without crm:team-read and forwards 14d cutoff", async () => {
      ...
      const result = await service.listStale(USER_ID, ["sales-revenue:read"], {
        page: 1,
        limit: 20,
      });
  ...
  describe("getById", () => {
    it("throws NotFound when row missing", async () => {
      ...
      await expect(
        service.getById("x", USER_ID, ["sales-revenue:read"]),
    ...
    it("hides leads owned by other users from non-team-read callers", async () => {
      ...
      await expect(
        service.getById("lead-1", USER_ID, ["sales-revenue:read"]),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe("update", () => {
    it("rejects edits on a converted lead", async () => {
      ...
      await expect(
        service.update("lead-1", USER_ID, ["sales-revenue:update"], {
          notes: "x",
        }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("nulls cleared optional fields", async () => {
      ...
      await service.update("lead-1", USER_ID, ["sales-revenue:update"], {
        email: undefined,
        phone: "",
      });
  ...
  describe("disqualify", () => {
    ...
    it("rejects disqualifying a converted lead", async () => {
      findById.mockResolvedValue({ ...baseLead, status: "converted" });
      await expect(
        service.disqualify("lead-1", USER_ID, ["sales-revenue:update"], {
          reason: "x",
        }),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("delete", () => {
    it("looks up the lead first to enforce ownership", async () => {
      ...
      await service.delete("lead-1", USER_ID, ["sales-revenue:delete"]);
    ...
    it("throws NotFound when caller does not own the row", async () => {
      findById.mockResolvedValue({ ...baseLead, ownerId: OTHER_USER_ID });
      await expect(
        service.delete("lead-1", USER_ID, ["sales-revenue:delete"]),
      ).rejects.toThrow(NotFoundException);
