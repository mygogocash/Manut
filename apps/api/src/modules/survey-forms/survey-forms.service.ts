...
// as a single SystemSetting row (independent of the Survey module's list).
...
export class SurveyFormsService {
  async list(
    ...
    if (scope === "available") {
      ...
      const myResponses =
        formIds.length > 0
          ? await prisma.surveyFormResponse.findMany({
  ...
  async create(userId: string, input: CreateSurveyFormInput) {
  ...
  async update(id: string, userId: string, input: UpdateSurveyFormInput) {
    const existing = await prisma.surveyForm.findUnique({
      where: { id },
    ...
    return prisma.surveyForm.update({
      where: { id },
      data: {
  ...
  async replaceQuestions(
    ...
    return prisma.$transaction(async (tx) => {
      await tx.surveyFormQuestion.deleteMany({ where: { surveyFormId: id } });
      ...
      return tx.surveyForm.findUnique({
        where: { id },
        include: FORM_INCLUDES,
      });
    });
  ...
  async delete(id: string, userId: string) {
    ...
    return prisma.surveyForm.delete({ where: { id } });
  }
  ...
  async publish(
    ...
    const updated = await prisma.surveyForm.update({
      where: { id },
  ...
  // controls any survey manager (admin / HR via `survey:manage-wave`) can use,
  ...
  private assertCanManageStatus(
    ...
    if (
      ...
      !permissions.includes(PERMISSIONS.SURVEY_MANAGE_WAVE)
    ) {
  ...
  async submitResponse(id: string, userId: string, input: SubmitResponseInput) {
    ...
    if (!form.isAnonymous) {
      const existing = await prisma.surveyFormResponse.findUnique({
        where: {
    ...
    const saved = await prisma.$transaction(async (tx) => {
      ...
      return tx.surveyFormResponse.findUnique({
  ...
  private async notifyFormSubmission(
      ...
      const recipients = await prisma.user.findMany({
        where: {
          ...
          OR: [
            ...
            {
              userRoles: {
                some: { role: { name: { in: ["Admin", "HR Manager"] } } },
              },
            },
          ],
        },
    ...
    const responseCount = await prisma.surveyFormResponse.count({
      where: { surveyFormId: form.id },
    });
    ...
    const email = surveyFormSubmittedEmail({
      ...
      responseCount,
      portalUrl: `${PORTAL_URL}/survey-forms/${form.id}`,
    });
  ...
  async getMyResponse(id: string, userId: string) {
    ...
    return prisma.surveyFormResponse.findUnique({
      where: {
        surveyFormId_respondentId: {
          surveyFormId: id,
          respondentId: userId,
  ...
  async listResponses(id: string, userId: string, userPermissions: string[]) {
    ...
    const responses = await prisma.surveyFormResponse.findMany({
  ...
  async getAnalytics(id: string, userId: string, userPermissions: string[]) {
    ...
    const allAnswers = await prisma.surveyFormAnswer.findMany({
      where: { question: { surveyFormId: id } },
  ...
  private questionInputToCreate(q: SurveyFormQuestionInput, order: number) {
    return {
      order,
      type: q.type,
      prompt: q.prompt,
...
export const surveyFormsService = new SurveyFormsService();
