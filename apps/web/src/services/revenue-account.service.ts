...
export async function listAccounts(
  ...
  return api.get(`/sales-revenue/accounts${buildQuery(params)}`);
}

export async function getAccount(
  id: string,
): Promise<ApiSuccessResponse<Account>> {
  return api.get(`/sales-revenue/accounts/${id}`);
}

export async function createAccount(
  input: CreateAccountInput,
): Promise<ApiSuccessResponse<Account>> {
  return api.post("/sales-revenue/accounts", input);
}
...
export async function updateAccount(
  ...
  return api.put(`/sales-revenue/accounts/${id}`, input);
}

export async function deleteAccount(id: string): Promise<void> {
  await api.delete(`/sales-revenue/accounts/${id}`);
}

export async function reorderAccounts(
  orderedIds: string[],
): Promise<ApiSuccessResponse<{ success: boolean }>> {
  return api.post("/sales-revenue/accounts/reorder", { orderedIds });
}

export async function importAccounts(
  rows: CreateAccountInput[],
): Promise<ApiSuccessResponse<{ created: number; skipped: number }>> {
  return api.post("/sales-revenue/accounts/import", { rows });
}
