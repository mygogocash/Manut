...
export async function listLeadSources(
  ...
  return api.get(`/sales-revenue/lead-sources${buildQuery(params)}`);
}

export async function createLeadSource(
  input: CreateLeadSourceInput,
): Promise<ApiSuccessResponse<LeadSource>> {
  return api.post("/sales-revenue/lead-sources", input);
}
...
export async function updateLeadSource(
  ...
  return api.put(`/sales-revenue/lead-sources/${id}`, input);
}

export async function deleteLeadSource(id: string): Promise<void> {
  await api.delete(`/sales-revenue/lead-sources/${id}`);
}
