...
// Legal team's three-state workflow (matches the Google Sheet they
// were using before adoption, 2026-05-26).
...
interface Props {
  ...
  project: LegalProject | null;
  onSaved: (project: LegalProject) => void;
}

export function LegalCrmFormDialog({
  open,
  onOpenChange,
  users,
  project,
  ...
  useEffect(() => {
    ...
    if (project) {
      ...
      setPriority(normalizeLegalPriority(project.priority));
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const payload: CreateLegalProjectInput = {
        name: name.trim(),
      ...
      const res = project
        ? await updateLegalProject(project.id, payload)
        : await createLegalProject(payload);
      toast.success(project ? "Legal Task updated" : "Legal Task created");
      onSaved(res.data);
      onOpenChange(false);
    } catch (err) {
      const msg =
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {isEdit ? "Edit Project" : "New Legal Task"}
          </DialogTitle>
          <DialogDescription>
            {isEdit ? "Update project details." : "Create a new Legal Task."}
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="legal-workstream">Workstream</Label>
            <Input
              id="legal-workstream"
              value={workstream}
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="legal-name">Legal Task *</Label>
            <Input
              id="legal-name"
              value={name}
