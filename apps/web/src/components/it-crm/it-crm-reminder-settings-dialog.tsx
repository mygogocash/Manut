...
export function ItCrmReminderSettingsDialog({ open, onOpenChange }: Props) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await updateReminderSettings({ recipients: finalEmails });
      setEmails(res.data.recipients);
      setDraft("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Extra people to email for IT CRM deadline reminders, on top of each
            project owner and task assignees.
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="space-y-2 py-2">
            <Label htmlFor="it-reminder-recipient-input">Recipients</Label>
            <div className="flex flex-wrap gap-1.5 rounded-md border px-2 py-2">
              ...
              <Input
                ref={draftRef}
                id="it-reminder-recipient-input"
                value={draft}
