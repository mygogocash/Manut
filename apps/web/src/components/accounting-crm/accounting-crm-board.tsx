...
interface AccountingCrmBoardProps {
  refreshKey?: number;
  onEdit: (project: AccountingProject) => void;
  onCreate: () => void;
}
...
function SortableStatusColumn({
  ...
  const {
    ...
    data: { type: "accounting-board-column", status },
      ...
      >
        {cards.length === 0 ? (
          ...
          cards.map((p) => {
            ...
            const priority = normalizeAccountingPriority(p.priority);
            ...
            return (
              ...
              >
                ...
                <Badge
                  variant={ACCOUNTING_PRIORITY_VARIANTS[priority] ?? "grey"}
                  className="w-fit text-[10px]"
                >
                  {ACCOUNTING_PRIORITY_LABELS[priority] ?? priority}
                </Badge>
                <div
                  className={`
...
export function AccountingCrmBoard({
  refreshKey = 0,
  onEdit,
  onCreate,
}: AccountingCrmBoardProps) {
  const { hasPermission } = useAuth();
  const canMoveCards = hasPermission("accounting-crm:update");
  ...
  async function moveProject(id: string, target: string) {
    ...
    if (!card || normalizeAccountingStatus(card.status) === target) return;
    setProjects((prev) =>
      prev.map((p) => (p.id === id ? { ...p, status: target } : p)),
    );
    try {
      await updateAccountingProject(id, { status: target });
    } catch (err) {
  ...
  function handleCardDrop(e: React.DragEvent, status: string) {
    ...
    if (normalizeAccountingStatus(dragged.status) === status) {
  ...
  async function reorderCardToEnd(draggedId: string, status: string) {
    ...
    let lastIdx = -1;
    next.forEach((p, i) => {
      if (normalizeAccountingStatus(p.status) === status) lastIdx = i;
    });
  ...
  function handleColumnDragEnd(event: DragEndEvent) {
    ...
    const activeStatus = String(active.id).replace(
      "accounting-status-col-",
      "",
    );
    const overStatus = String(over.id).replace("accounting-status-col-", "");
    if (
      !DEFAULT_BOARD_COLUMN_ORDER.includes(activeStatus as AccountingStatus) ||
      !DEFAULT_BOARD_COLUMN_ORDER.includes(overStatus as AccountingStatus)
    ) {
      return;
    }
    reorderColumns(
      activeStatus as AccountingStatus,
      overStatus as AccountingStatus,
    );
  }

  const byStatus: Record<string, AccountingProject[]> = {};
  for (const s of STATUS_OPTIONS) byStatus[s.value] = [];
  for (const p of projects) {
    const key = normalizeAccountingStatus(p.status);
    (byStatus[key] ??= []).push(p);
  }
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <PermissionButton permission="accounting-crm:create" onClick={onCreate}>
