...
import { FALLBACK_ONBOARDING_TEMPLATE } from "@/components/hrms/hrms-constants";
import {
  cleanParts,
  type EditablePart,
  PartsEditor,
...
function slugKey(label: string): string {
  const slug = label
    ...
    .replace(/^_+|_+$/g, "")
    .slice(0, 60);
  return slug || `task_${Math.random().toString(36).slice(2, 7)}`;
}
...
export function OnboardingDialog({
  ...
  // On open: reset + seed parts from the admin template (fallback keeps
  // the dialog usable if the fetch fails).
  useEffect(() => {
    if (!open) return;
    setEmployeeId("");
    setEmployeeName("");
    ...
    setStartDate("");
    setEntityId("");
    setParts(FALLBACK_ONBOARDING_TEMPLATE.parts.map((p) => ({ ...p })));
    let cancelled = false;
  ...
  // Hydrate name / department / entity when linking an existing employee.
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.success("Onboarding run created");
      onSaved();
      onOpenChange(false);
    } catch (err) {
      toast.error(
        getErrorMessage(err, "We couldn't create the onboarding run."),
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle>Create onboarding run</DialogTitle>
          <DialogDescription>
            Set up an onboarding checklist for a new employee. Parts &amp; tasks
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-muted-foreground text-[11px]">
              Optional — leave empty if onboarding an external candidate not yet
              added to Employees.
            </p>
          </div>

          <div className="flex flex-col gap-1.5">
            <Label htmlFor="onb-name">Employee name *</Label>
            <Input
              id="onb-name"
