...
import { openOffboardingPrintView } from "@/components/hrms/offboarding-export";
...
export function OffboardingChecklist({
  ...
  // Client-only parts added on this run before they hold any task (a
  // part with no tasks can't persist, since every task carries its part).
  const [extraParts, setExtraParts] = useState<string[]>([]);
  const [editingPart, setEditingPart] = useState<string | null>(null);
  const [draftPart, setDraftPart] = useState("");
  const [savingBulk, setSavingBulk] = useState(false);
  const [signingParty, setSigningParty] = useState<"employee" | "hr" | null>(
    null,
  );
  ...
  // Distinct parts in first-appearance order, plus any locally-added
  // empty parts not yet backed by a task.
  ...
  function toInputs(): OffboardingTaskInput[] {
  ...
  async function commit(next: OffboardingTaskInput[]) {
  ...
  async function handleRenamePart(oldName: string) {
    ...
    if (affected.length === 0) return; // empty (extra) part — local rename only
    await commit(
  ...
  async function handleSignConfirm() {
    if (!onSign || !run || !signingParty) return;
    const name = signName.trim();
    if (!name) return;
    setSigningParty(null);
    await onSign(run.id, signingParty, name);
  }
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-foreground text-sm font-medium">
          Offboarding checklist for {employeeName}
        </h3>
        {savingBulk ? (
        ...
        <Button
          ...
          variant="outline"
          type="button"
          onClick={() => openOffboardingPrintView(run)}
        >
            ...
            {partTasks.map((task) => {
              ...
              return (
                ...
                >
                  ...
                  {task.done && task.doneAt && !isEditingThis && (
                    <span
                      className={`
      ...
      <div className="border-border grid grid-cols-2 gap-4 border-t pt-4">
        {(["employee", "hr"] as const).map((party) => {
          const signedName =
            party === "employee" ? run.employeeSignName : run.hrSignName;
          const signedAt =
            party === "employee" ? run.employeeSignedAt : run.hrSignedAt;
          const label = party === "employee" ? "Employee" : "HR";
          const isSigningThis = signingParty === party;
          return (
              ...
              {signedName ? (
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setSignName(
                      party === "employee" ? employeeName : currentUserName,
                    );
                  }}
                >
                  Sign as {label}
