...
const stepSchema = z
  .object({
    ...
    categoryFilter: z.array(z.enum(EXPENSE_CATEGORIES)),
...
interface ExpenseApprovalStepDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  step: ExpenseApprovalStep | null;
  users: UserListItem[];
  onSaved: () => void;
}

export function ExpenseApprovalStepDialog({
  open,
  onOpenChange,
  step,
  users,
  onSaved,
}: ExpenseApprovalStepDialogProps) {
  ...
  async function onSubmit(values: StepFormValues) {
    try {
      ...
      if (step) {
        await updateExpenseApprovalStep(step.id, payload);
        toast.success(`Step "${payload.name}" updated`);
      } else {
        await createExpenseApprovalStep(payload);
    ...
    >
      ...
      >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Limits this step to reports with one of the chosen
                    categories. Empty list = every category.
                  </FormDescription>
                  <FormControl>
                      ...
                      {EXPENSE_CATEGORIES.map((cat) => {
                        ...
                        return (
                          <button
                            ...
                            key={cat}
                            onClick={() => {
                              const next: ExpenseCategoryKey[] = checked
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Empty = no upper bound. Compared against the report total
                      converted to THB.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
        ...
        <DialogFooter>
          ...
          <Button
            type="submit"
            form="expense-approval-step-form"
            disabled={submitting}
          >
