...
import { exportSurveyResponsesCsv } from "@/components/survey/survey-form-export";
import { SurveyPublishDialog } from "@/components/survey/survey-form-publish-dialog";
...
export default function SurveyDetailPage() {
  ...
  async function doPublish(announce: SurveyAnnounceOptions) {
    try {
      ...
      await publishSurvey(survey.id, announce);
  ...
  async function doAnnounce(announce: SurveyAnnounceOptions) {
    try {
      setAnnouncing(true);
      const res = await announceSurvey(survey.id, announce);
      ...
      >
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => router.push("/survey")}>
            <ArrowLeft className="mr-1 size-3.5" /> All surveys
          </Button>
...
function ReadonlyOverview({ form }: { form: SurveyDetail }) {
  return (
    <div className="flex flex-col gap-4">
...
function AnalyticsView({ analytics }: { analytics: SurveyAnalytics | null }) {
  if (!analytics) {
    return (
      <p className="text-muted-foreground py-8 text-center text-sm">
        Loading analytics…
