...
export const appraisalQuerySchema = z.object({
  ...
  status: z
    ...
    .optional(),
  /**
   * Free-text match over the employee's name or email.
   *
   * The Appraisals table has had a "Search by employee" box for as long as it has
   * existed, but the term was never sent anywhere — it only reset the page
   * number — so typing a name jumped to page 1 and changed nothing.
   *
   * Blank collapses to `undefined`: the UI sends whatever is in the box, and an
   * empty box means "no filter" rather than "match the empty string".
   */
  search: z
    .string()
    .trim()
