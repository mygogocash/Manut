...
// Condition ladder for furniture. Kept in the application rather than a DB enum
// so facilities can be given another rung without a migration.
...
export const createAssetSchema = z.object({
  ...
  // Photo — any category, not just furniture.
  ...
  condition: z.enum(ASSET_CONDITIONS).optional().or(z.literal("")),
...
/**
 * A date on an imported row. Accepts YYYY-MM-DD only — the FRONTEND normalises
 * whatever the sheet used (this tracker writes DD-MM-YYYY) before posting, so a
 * value reaching here in another shape is a mapper bug and must fail loudly
...
export const assetImportRowSchema = z.object({
  ...
  // ── Fixed-asset columns ──
  // These are the substance of a purchase log, and the importer used to carry
  // none of them: a furniture tracker went through it and arrived as names with
  // every price, date and quantity silently discarded. `createAssetSchema` has
  // supported them all along, so only the bulk path was short.
  ...
  warrantyUntil: importDate,
  // Furniture metadata. A desk has no serial number or operating system; what
  // identifies it is what it is made of, how big it is and where it stands.
  material: z.string().max(120).optional().nullable(),
  dimensions: z.string().max(120).optional().nullable(),
  condition: z.enum(ASSET_CONDITIONS).optional().nullable().or(z.literal("")),
  locationDetail: z.string().max(200).optional().nullable(),
  // Writeable so finance can supply their own register code — which also makes
  // it the strongest idempotency key an import can carry.
  assetCode: z.string().max(120).optional().nullable(),
});

/**
 * Which office the imported rows belong to.
 *
 * Without this the importer resolved an office from the ASSIGNEE's entity
 * country and fell back to "the first active office" for rows with no assignee
 * — silently correct for an IT hand-out sheet and silently wrong for a
 * furniture log, where no row has an assignee at all.
 *
 * `officeId` targets an existing office. `name` + `city` + `country` find it by
 * name or create it (both columns are non-nullable on Office, so all three are
 * required together). Omitted entirely, the legacy country/fallback resolution
 * still applies so existing callers are unaffected.
 */
export const assetImportOfficeSchema = z
  .object({
  ...
  .refine(
    ...
    {
      message:
        "Provide officeId, or all of name + city + country to create an office",
    },
  );

export const assetImportSchema = z.object({
  rows: z.array(assetImportRowSchema).min(1, "rows is required").max(2000),
  office: assetImportOfficeSchema.optional(),
});

export type AssetImportRow = z.infer<typeof assetImportRowSchema>;
export type AssetImportOffice = z.infer<typeof assetImportOfficeSchema>;
