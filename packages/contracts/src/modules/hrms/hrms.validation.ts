...
function refineValueRequirements<T extends z.ZodTypeAny>(schema: T) {
  return (
    schema
      .refine(
        (v: z.infer<T>) =>
        ...
        { message: "Shares must be greater than 0", path: ["shares"] },
      )
      .refine(
        (v: z.infer<T>) =>
          v.valueType !== "currency" ||
          (Boolean(v.currencyCode) &&
            typeof v.currencyAmount === "number" &&
            v.currencyAmount > 0),
        {
      ...
      .refine(
        (v: z.infer<T>) =>
          v.valueType !== "percent" ||
          (typeof v.percentOfBase === "number" && v.percentOfBase > 0),
        {
      ...
      .refine(
        (v: z.infer<T>) =>
          v.allocationMode !== "monthly_recurring" ||
          (Boolean(v.allocationStartMonth) && Boolean(v.allocationEndMonth)),
        {
      ...
      // One-time grants drive their vesting schedule off Start/End: provide
      // both (vests over a schedule) or neither (granted outright). A single
      // date can't be turned into a schedule, so reject the half-filled case.
      .refine(
        (v: z.infer<T>) =>
          v.allocationMode !== "one_time" ||
          Boolean(v.allocationStartMonth) === Boolean(v.allocationEndMonth),
        {
          message:
...
export const createEsopGrantSchema = refineValueRequirements(
  baseGrantFields.extend({
    employeeId: z.string().uuid("Invalid employee ID"),
    grantDate: dateString,
  }),
);

export const updateEsopGrantSchema = refineValueRequirements(
  baseGrantFields.partial().extend({
