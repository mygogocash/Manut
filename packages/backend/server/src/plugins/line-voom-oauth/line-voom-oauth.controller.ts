...
@Controller('/oauth/line-voom')
export class LineVoomOAuthController {
  private readonly logger = new Logger(LineVoomOAuthController.name);

  constructor(private readonly lineVoom: LineVoomOAuthService) {}
  ...
  async callback(
    ...
    try {
      const { displayName } = await this.lineVoom.handleCallback(code, state);
      return res
        .status(HttpStatus.OK)
        .send(this.renderResultPage({ ok: true, displayName }));
    } catch (err) {
      this.logger.error(
        'LINE VOOM OAuth callback failed',
  ...
  private renderResultPage(
    ...
    const fallbackQuery = result.ok
      ? `connected=line-voom&name=${encodeURIComponent(result.displayName)}`
