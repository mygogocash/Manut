 ...
 *
 * Lives at `/oauth/figma/callback` — Figma's OAuth app config
...
export class FigmaOAuthController {
  ...
  async callback(
    ...
    try {
      const { handle } = await this.figma.handleCallback(code, state);
      return res
        .status(HttpStatus.OK)
        .send(this.renderResultPage({ ok: true, handle }));
    } catch (err) {
      this.logger.error(
  ...
  private renderResultPage(
    result: { ok: true; handle: string } | { ok: false; error: string }
  ): string {
    const fallbackQuery = result.ok
      ? `connected=figma&handle=${encodeURIComponent(result.handle)}`
