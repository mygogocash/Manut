...
const POSTMESSAGE_TYPE = 'affine:threads-oauth-result';

@Controller('/oauth/threads')
export class ThreadsOAuthController {
  private readonly logger = new Logger(ThreadsOAuthController.name);

  constructor(private readonly threads: ThreadsOAuthService) {}
  ...
  async callback(
    ...
    try {
      const { username } = await this.threads.handleCallback(code, state);
      return res
  ...
  private renderResultPage(
    ...
    const fallbackQuery = result.ok
      ? `connected=threads&username=${encodeURIComponent(result.username)}`
