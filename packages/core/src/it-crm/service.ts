...
const members = schema.itProjectMembers;
const columns = schema.itProjectColumns;
const owner = alias(schema.users, "it_crm_owner");
...
export async function list(db: Db, userId: string, perms: string[], query: ItProjectQuery) {
  const canSeeAll =
...
export async function create(db: Db, userId: string, input: CreateItProjectInput) {
  ...
  return getById(db, id, userId, [PERMISSIONS.IT_CRM_READ]);
}

export async function importRows(db: Db, userId: string, rows: CreateItProjectInput[]) {
  let created = 0;
...
export async function update(
  ...
  if (input.status !== undefined) {
    ...
    if (input.status !== existing.status) patch.statusChangedAt = nowIso();
  }
  if (input.healthStatus !== undefined) patch.healthStatus = input.healthStatus;
  if (input.effortPoints !== undefined) patch.effortPoints = input.effortPoints;
  ...
  if (input.goLiveDate !== undefined || input.revisedGoLiveDate !== undefined) {
    patch.remindersSent = [];
...
export async function reorder(db: Db, input: ReorderItProjectsInput) {
  const ts = nowIso();
  await db.transaction(async (tx) => {
...
export async function getReminderRecipients(db: Db) {
  const value = await getSetting(db, "it-crm.reminder_recipients");
  const raw =
...
export async function setReminderRecipients(db: Db, input: { recipients: string[] }) {
  ...
  await upsertSetting(db, "it-crm.reminder_recipients", { recipients: [...new Set(recipients)] });
