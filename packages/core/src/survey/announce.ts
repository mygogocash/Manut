...
import type { PublishAnnounceInput } from "@nexora/contracts/modules/survey/survey.validation";
...
import * as surveyRepo from "./survey.repository";
...
export async function announcePublishedForm(
  ...
  const defaults = await getAnnouncementDefaults(db);
  ...
  if (announce.wall && permissions.includes(PERMISSIONS.WALL_CREATE)) {
    try {
      ...
      console.warn(JSON.stringify({ level: "warn", event: "survey_announce_wall_failed", formId: form.id, err: String(err) }));
    }
  }
  ...
  if (announce.news && permissions.includes(PERMISSIONS.NEWS_CREATE)) {
    try {
      ...
      console.warn(JSON.stringify({ level: "warn", event: "survey_announce_news_failed", formId: form.id, err: String(err) }));
    }
  }
  ...
  if (announce.companyDate && announce.deadline && permissions.includes(PERMISSIONS.ADMIN_MANAGE)) {
    try {
      ...
      console.warn(JSON.stringify({ level: "warn", event: "survey_announce_date_failed", formId: form.id, err: String(err) }));
    }
  }
