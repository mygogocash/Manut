...
const projects = schema.legalProjects;
const members = schema.legalProjectMembers;
const columns = schema.legalProjectColumns;
const owner = alias(schema.users, "legal_crm_owner");
...
export async function list(db: Db, userId: string, perms: string[], query: LegalProjectQuery) {
  const canSeeAll =
...
export async function create(db: Db, userId: string, input: CreateLegalProjectInput) {
  ...
  return getById(db, id, userId, [PERMISSIONS.LEGAL_CRM_READ]);
}

export async function importRows(db: Db, userId: string, rows: CreateLegalProjectInput[]) {
  let created = 0;
...
export async function reorder(db: Db, input: ReorderLegalProjectsInput) {
  const ts = nowIso();
  await db.transaction(async (tx) => {
...
export async function getReminderRecipients(db: Db) {
  const value = await getSetting(db, "legal-crm.reminder_recipients");
  const raw =
...
export async function setReminderRecipients(db: Db, input: { recipients: string[] }) {
  ...
  await upsertSetting(db, "legal-crm.reminder_recipients", { recipients: [...new Set(recipients)] });
