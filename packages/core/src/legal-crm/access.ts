...
export async function requireMembership(
  ...
  const [member] = await db
    .select({ id: schema.legalProjectMembers.id })
    .from(schema.legalProjectMembers)
    .where(
      and(
        eq(schema.legalProjectMembers.projectId, project.id),
        eq(schema.legalProjectMembers.userId, userId),
      ),
    )
    .limit(1);
...
export function requireOwnerOrManage(role: ProjectRole, perms: string[]): void {
  ...
  if (perms.includes(PERMISSIONS.LEGAL_CRM_MANAGE) || perms.includes(PERMISSIONS.PROJECTS_MANAGE)) {
    return;
  }
