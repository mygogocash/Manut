...
export async function requireMembership(
  ...
  const [member] = await db
    .select({ id: schema.productProjectMembers.id })
    .from(schema.productProjectMembers)
    .where(
      and(
        eq(schema.productProjectMembers.projectId, project.id),
        eq(schema.productProjectMembers.userId, userId),
      ),
    )
    .limit(1);
...
export function requireOwnerOrManage(role: ProjectRole, perms: string[]): void {
  ...
  if (perms.includes(PERMISSIONS.PRODUCT_CRM_MANAGE) || perms.includes(PERMISSIONS.PROJECTS_MANAGE)) {
    return;
  }
