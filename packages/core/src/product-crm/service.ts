...
const projects = schema.productProjects;
const members = schema.productProjectMembers;
const columns = schema.productProjectColumns;
const owner = alias(schema.users, "product_crm_owner");
...
export async function list(db: Db, userId: string, perms: string[], query: ProductProjectQuery) {
  const canSeeAll =
...
export async function create(db: Db, userId: string, input: CreateProductProjectInput) {
  ...
  return getById(db, id, userId, [PERMISSIONS.PRODUCT_CRM_READ]);
}

export async function importRows(db: Db, userId: string, rows: CreateProductProjectInput[]) {
  let created = 0;
...
export async function reorder(db: Db, input: ReorderProductProjectsInput) {
  const ts = nowIso();
  await db.transaction(async (tx) => {
...
export async function getReminderRecipients(db: Db) {
  const value = await getSetting(db, "product-crm.reminder_recipients");
  const raw =
...
export async function setReminderRecipients(db: Db, input: { recipients: string[] }) {
  ...
  await upsertSetting(db, "product-crm.reminder_recipients", { recipients: [...new Set(recipients)] });
