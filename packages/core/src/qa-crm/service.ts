...
const projects = schema.qaProjects;
const members = schema.qaProjectMembers;
const columns = schema.qaProjectColumns;
const owner = alias(schema.users, "qa_crm_owner");
...
export async function list(db: Db, userId: string, perms: string[], query: QaProjectQuery) {
  const canSeeAll =
...
export async function create(db: Db, userId: string, input: CreateQaProjectInput) {
  ...
  return getById(db, id, userId, [PERMISSIONS.QA_CRM_READ]);
}

export async function importRows(db: Db, userId: string, rows: CreateQaProjectInput[]) {
  let created = 0;
...
export async function reorder(db: Db, input: ReorderQaProjectsInput) {
  const ts = nowIso();
  await db.transaction(async (tx) => {
...
export async function getReminderRecipients(db: Db) {
  const value = await getSetting(db, "qa-crm.reminder_recipients");
  const raw =
...
export async function setReminderRecipients(db: Db, input: { recipients: string[] }) {
  ...
  await upsertSetting(db, "qa-crm.reminder_recipients", { recipients: [...new Set(recipients)] });
