...
import type {
  ...
  ListSurveyFormsQuery,
  NotificationSettingsInput,
  PublishAnnounceInput,
  ReplaceQuestionsInput,
  ScheduleSurveyFormInput,
  SubmitResponseInput,
  SurveyFormQuestionInput,
...
import * as repo from "./survey-forms.repository";
import * as settingsRepo from "../survey/system-settings.repository";

const ANNOUNCE_SETTINGS_KEY = "survey.announcement_defaults";
const NOTIFY_SETTINGS_KEY = "survey.notification_recipients";

function canManage(perms: string[]) {
  return perms.includes(PERMISSIONS.SURVEY_MANAGE_WAVE);
}

function questionInputToCreate(q: SurveyFormQuestionInput, order: number) {
  return {
    order,
    type: q.type,
    prompt: q.prompt,
...
export async function list(db: Db, userId: string, userPermissions: string[], query: ListSurveyFormsQuery) {
  ...
  const { data, total } = await repo.findSurveyForms(db, filters, page, limit);
  ...
  if (scope === "available") {
    ...
    const respondedSet = new Set(myResponses.map((r) => r.surveyFormId));
    return {
...
export async function getById(db: Db, id: string, userId: string, userPermissions: string[]) {
  const form = await repo.findSurveyFormById(db, id);
...
export async function create(db: Db, userId: string, input: CreateSurveyFormInput) {
  return repo.createSurveyForm(db, {
    title: input.title,
...
export async function update(db: Db, id: string, userId: string, input: UpdateSurveyFormInput) {
  const existing = await repo.findSurveyFormMeta(db, id);
  ...
  return repo.updateSurveyForm(db, id, {
...
export async function setSchedule(db: Db, id: string, userId: string, input: ScheduleSurveyFormInput) {
  const existing = await repo.findSurveyFormMeta(db, id);
  if (!existing) throw new NotFoundException("Survey form not found");
...
export async function replaceQuestions(db: Db, id: string, userId: string, input: ReplaceQuestionsInput) {
  ...
  const updated = await repo.replaceSurveyFormQuestions(
    db,
    id,
...
export async function remove(db: Db, id: string, userId: string) {
  ...
  await repo.deleteSurveyForm(db, id);
}
...
export async function publish(
  ...
  const qCount = await repo.countSurveyFormQuestions(db, id);
  if (qCount === 0) throw new BadRequestException("Add at least one question before publishing");

  const updated = await repo.updateSurveyForm(db, id, {
    status: "published",
...
export async function announceNow(
  ...
  const form = await repo.findSurveyFormMeta(db, id);
  if (!form) throw new NotFoundException("Survey form not found");
...
function assertCanManageStatus(
  ...
  if (existing.createdById !== userId && !permissions.includes(PERMISSIONS.SURVEY_MANAGE_WAVE)) {
...
export async function close(db: Db, id: string, userId: string, permissions: string[]) {
  ...
  return repo.updateSurveyForm(db, id, { status: "closed", closedAt: new Date().toISOString() });
}
...
export async function reopen(db: Db, id: string, userId: string, permissions: string[]) {
  ...
  return repo.updateSurveyForm(db, id, { status: "published", closedAt: null });
}
...
export async function archive(db: Db, id: string, userId: string) {
  ...
  return repo.updateSurveyForm(db, id, { archivedAt: new Date().toISOString() });
}
...
export async function unarchive(db: Db, id: string, userId: string) {
  ...
  return repo.updateSurveyForm(db, id, { archivedAt: null });
}

export async function submitResponse(db: Db, id: string, userId: string, input: SubmitResponseInput) {
  const form = await repo.findSurveyFormWithQuestions(db, id);
  if (!form) throw new NotFoundException("Survey form not found");
  ...
  const saved = await repo.createSurveyFormResponse(db, {
  ...
  void notifyFormSubmission(db, form, userId).catch((err) => {
    console.error(JSON.stringify({ level: "error", event: "survey_forms_submission_notify_failed", formId: form.id, err: String(err) }));
  });
...
async function notifyFormSubmission(
  ...
  const responseCount = await repo.countSurveyFormResponses(db, form.id);
  void responseCount;
  void respondentName;
...
export async function listResponses(db: Db, id: string, userId: string, userPermissions: string[]) {
  ...
  return repo.listSurveyFormResponses(db, id, form.isAnonymous);
}
...
export async function getAnalytics(db: Db, id: string, userId: string, userPermissions: string[]) {
  ...
  const allAnswers = await repo.listSurveyFormAnswersForAnalytics(db, id);
