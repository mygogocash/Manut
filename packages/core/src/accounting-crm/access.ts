...
export async function requireMembership(
  ...
  const [member] = await db
    .select({ id: schema.accountingProjectMembers.id })
    .from(schema.accountingProjectMembers)
    .where(
      and(
        eq(schema.accountingProjectMembers.projectId, project.id),
        eq(schema.accountingProjectMembers.userId, userId),
      ),
    )
    .limit(1);
...
export function requireOwnerOrManage(role: ProjectRole, perms: string[]): void {
  ...
  if (perms.includes(PERMISSIONS.ACCOUNTING_CRM_MANAGE) || perms.includes(PERMISSIONS.PROJECTS_MANAGE)) {
    return;
  }
