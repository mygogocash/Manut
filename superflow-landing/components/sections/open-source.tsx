...
const POINTS = [
  ...
  },
  {
    ...
    body: "Tracks the AFFiNE canary branch upstream and ships Superflow features on top.",
  },
] as const;

const COMMITS = [
  { color: "oklch(0.7_0.18_140)", text: "feat: AI write tools and Mode picker", time: "2d ago" },
  { color: "oklch(0.7_0.16_280)", text: "fix: SSE parser and auto-tag accuracy", time: "4d ago" },
  { color: "oklch(0.7_0.16_240)", text: "feat: Calendar integration (Google, CalDAV)", time: "1w ago" },
  { color: "oklch(0.78_0.18_85)", text: "feat: Multi-model auto-routing (Llama, Mistral)", time: "2w ago" },
] as const;
      ...
      <div className="container-prose grid gap-12 md:grid-cols-2 md:gap-20">
        <Reveal>
          ...
          <p className="mt-5 max-w-prose text-pretty text-base text-muted-foreground sm:text-lg">
            Superflow is an open-source fork of AFFiNE — one of the most starred productivity repos on GitHub. Every line is auditable. Every deployment is yours.
          </p>
        ...
        <Reveal delay={140}>
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border">
            {[
              ...
              { num: "MIT", label: "License", color: "text-emerald-500 dark:text-emerald-400" },
              { num: "∞", label: "Seat limit", color: "text-foreground" },
              { num: stats.release, label: "Latest release", color: "text-foreground" },
            ].map((s, i) => (
          ...
          <div className="mt-5 rounded-2xl border border-border bg-card p-6">
            ...
            <ul className="space-y-3">
              {COMMITS.map((c) => (
                <li key={c.text} className="flex items-center gap-3 text-[13px]">
                  <span
                    aria-hidden
