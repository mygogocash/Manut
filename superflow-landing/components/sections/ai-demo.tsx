...
const MODE_TOOLS: Record<Mode, ReadonlyArray<string>> = {
  ...
  agent: ["Search workspace", "Read docs", "Edit docs", "Create docs", "Data views"],
};
...
const MODE_REPLY: Record<Mode, string> = {
  read:
    "I found 3 meeting docs from last week. Here is a summary you can copy. I will not write to any document in Read-only mode.",
  edit:
    "I summarized 3 meeting docs and edited the open document with the action plan. I left the doc structure intact and added a new heading at the top.",
  agent:
      ...
      <div className="container-prose grid items-start gap-12 md:grid-cols-2 md:gap-16">
        <Reveal>
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[0_20px_60px_-20px_oklch(0_0_0/0.18)] dark:shadow-[0_20px_60px_-20px_oklch(0_0_0/0.55)]">
            ...
            <div className="flex items-center gap-3 border-b border-border bg-muted/40 px-5 py-3.5">
              ...
              <span className="text-[13px] font-semibold tracking-tight">Superflow AI</span>

              <div role="tablist" aria-label="AI mode" className="ml-auto flex gap-1">
            ...
            <div className="flex flex-col gap-4 p-5">
              <div className="ml-auto max-w-[85%] rounded-2xl rounded-br-md bg-foreground px-3.5 py-2.5 text-[13px] text-background">
                Summarize last week&apos;s meeting notes and draft a follow-up action plan.
              </div>
              ...
              <div className="max-w-[92%] rounded-2xl rounded-bl-md border border-border bg-muted/40 px-4 py-3 text-[13px] leading-relaxed">
                <div className="mb-1.5 flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                  <Sparkles className="size-3" aria-hidden /> Superflow AI · {MODE_LABELS[mode]}
                </div>
                {typed ? (
                  <>
            ...
            <div className="mx-5 mb-3 flex items-center gap-2 rounded-xl border border-border bg-background px-3.5 py-2.5 text-[13px] text-muted-foreground">
              Ask anything or give Superflow AI a task…
            </div>
        ...
        <Reveal delay={120}>
          ...
          <p className="mt-5 max-w-prose text-pretty text-base text-muted-foreground sm:text-lg">
            Superflow AI is a true agent. It reads your workspace, writes to your docs, creates new pages, filters databases, and tags content. Powered by Gemini, Claude, and Llama on Vertex AI.
          </p>
