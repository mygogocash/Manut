...
import type { ListViewColumn, ListViewData } from './define.js';
...
export class ListSingleView extends SingleViewBase<ListViewData> {
  ...
  propertyGetOrCreate(propertyId: string): ListColumn {
    return new ListColumn(this, propertyId);
  }
  ...
  /** Property id currently used to group rows, or undefined for ungrouped. */
  groupBy$ = computed(() => this.data$.value?.groupBy);

  setGroupBy(propId: string | undefined) {
    this.dataUpdate(() => ({ groupBy: propId }));
  }

  // ─── Ordered rows ─────────────────────────────────────────────────────

  /** Rows filtered by the active filter, preserving data-source order. */
  orderedRows$ = computed(() => {
    return this.rows$.value.filter(row => this.isShow(row.rowId));
  });
  ...
  rowMove(rowId: string, targetPosition: InsertToPosition) {
    this.dataSource.rowMove(rowId, targetPosition);
  }

  // ─── Materialize stored columns vs live property set ──────────────────

  private materializeColumns() {
    const view = this.view;
    if (!view) return;
    const next = materializeListColumns(
      view.columns,
...
export class ListColumn extends PropertyBase {
  constructor(
    public listView: ListSingleView,
    propertyId: string
  ) {
    super(listView as any, propertyId);
  }
